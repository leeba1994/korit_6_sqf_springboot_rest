...
public class BasicController {
    ...
    * 1. 요청에 대한 동작은 무조건 메소드에 따라 정의하자. -> 약속
    *  - 데이터 추가(등록)은 POST 요청으로 하자, 그리고 POST요청은 JSON데이터(표준적으로)로 요청하자.
    *  - 데이터 조회는 GET 요청으로 하자,
...
//    public Object basicPost(@RequestBody ReqStudentDto reqStudentDto) { //@RequestBody JSON으로 받음
    public ResponseEntity<?> basicPost(@RequestBody ReqStudentDto reqStudentDto) { //@RequestBody JSON으로 받음
        log.info("Student : {}", reqStudentDto);  //표현식이다

        return ResponseEntity.badRequest().body(null); // ok -> 상태코드 200
//        return ResponseEntity.badRequest().body("응답데이터"); // ok -> 상태코드 200
//        return reqStudentDto;
    ...
    public Object basicStudentPost(@RequestBody ReqTeacherDto reqTeacherDto) {
        log.info("Teacher : {}", reqTeacherDto);
        return reqTeacherDto;
    }
